...
The first workflow I made is `get-semantic-verion.yml`. This looks at a branch and determines the new version of that branch based on tags that indicate previous (pre-release) versions and the commit messages since the most recent version. It again depends on commit conventions. In fact, my implementation actually adheres to the way [conventional commits](https://www.conventionalcommits.org/en/v1.0.0/) handles breaking changes with and exclamation mark, unlike the previous implementation. It assumes main is the release branch and assumes any other as pre-release branches.

[Here](https://docs.github.com/en/actions/how-tos/reuse-automations/reuse-workflows#using-outputs-from-a-reusable-workflow), I learnt how to work with reusing workflows. The implementation of `get-semantic-verion.yml` took quite some trial and error. To test the bash script in the `run` section, I created a temporary script file called `test.sh` where I pasted the script. I changed `${{ inputs.branch }}` to the branch I wanted to test it on and I put `set -e` at the top, because that is also what GitHub does. I then tested the script by running `GITHUB_OUTPUT="test-out.txt" ./test.sh`.
